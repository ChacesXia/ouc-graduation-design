<?php
    ...
    require_once('./include/db_info.inc.php');
        ...
        $view_title= $MSG_test.$MSG_RANKLIST;
        $title="";
...
$sql="SELECT `start_time`,`title`,`end_time` FROM `test` WHERE `test_id`='$cid'";
...
$sql="SELECT count(1) as pbc FROM `test_problem` WHERE `test_id`='$cid'";
...
$sql="SELECT
        users.user_id,users.nick,solution.result,solution.num,unix_timestamp(solution.in_date)-$start_time in_date
                FROM
                        (select * from solution where solution.test_id='$cid' and num>=0 and problem_id>0) solution
                left join users
...
$sql="select num,user_id from
        (select num,user_id from solution where test_id=$cid and result=4 order by solution_id ) test
        group by num";
if($OJ_MEMCACHE){
//        require("./include/memcache.php");
...
/////////////////////////Template
require("template/".$OJ_TEMPLATE."/testrank2.php");
