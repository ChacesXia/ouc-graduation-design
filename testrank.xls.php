...
$sql="SELECT `start_time`,`title` FROM `test` WHERE `test_id`='$cid'";
...
if ($rows_cnt>0){
  ...
  header ( "content-disposition:   attachment;   filename=test".$cid."_".$title.".xls" );
}
...
$sql="SELECT count(1) FROM `test_problem` WHERE `test_id`='$cid'";
...
//echo "<title>test RankList -- $title</title>";
echo "<center><h3>test RankList -- $title</h3></center>";
